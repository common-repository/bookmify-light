...
class HelperNotifications
{
	 ...
	 * @access public
	*/
	public static function notificationsListForCustomer()
	{
		global $wpdb;
		
		$query 	= "SELECT * FROM {$wpdb->prefix}bmify_notifications WHERE to_customer=1";
		...
		foreach($result as $notifications){
			...
			$wpEditor 					= Helper::bookmifyBeWpHtmlEditor($notificationID, $notificationMessage);
			
			$cronClass					= 'notification_item';
			if($notificationCron == 1){
				$cronClass				= 'notification_item notification_cron_item';
			}
	 ...
	 * @access private
	*/
    public static function emailTestHtml(){
        global $wpdb;
		$html = '';
	 ...
	 * @access public
	*/
	public static function notificationsListForEmployee()
	{
		global $wpdb;
		
		$query 	= "SELECT * FROM {$wpdb->prefix}bmify_notifications WHERE to_employee=1";
		$result = $wpdb->get_results( $query );
