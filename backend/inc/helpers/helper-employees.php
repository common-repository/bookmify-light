...
class HelperEmployees
{
	...
	*/
	...
	public static function getWorkingHoursEmployeeTab($ID = ''){
		...
		if($mo_wh_s == ''){$mo_wh_s = get_option('bookmify_be_monday_start', '08:00');}else{$moC1='data-checked="checked"';$moC2='checked';}
		if($mo_wh_e == ''){$mo_wh_e = get_option('bookmify_be_monday_end', '18:00');}
		if($tu_wh_s == ''){$tu_wh_s = get_option('bookmify_be_tuesday_start', '08:00');}else{$tuC1='data-checked="checked"';$tuC2='checked';}
		if($tu_wh_e == ''){$tu_wh_e = get_option('bookmify_be_tuesday_end', '18:00');}
		if($we_wh_s == ''){$we_wh_s = get_option('bookmify_be_wednesday_start', '08:00');}else{$weC1='data-checked="checked"';$weC2='checked';}
		if($we_wh_e == ''){$we_wh_e = get_option('bookmify_be_wednesday_end', '18:00');}
		if($th_wh_s == ''){$th_wh_s = get_option('bookmify_be_thursday_start', '08:00');}else{$thC1='data-checked="checked"';$thC2='checked';}
		if($th_wh_e == ''){$th_wh_e = get_option('bookmify_be_thursday_end', '18:00');}
		if($fr_wh_s == ''){$fr_wh_s = get_option('bookmify_be_friday_start', '08:00');}else{$frC1='data-checked="checked"';$frC2='checked';}
		if($fr_wh_e == ''){$fr_wh_e = get_option('bookmify_be_friday_end', '18:00');}
		if($sa_wh_s == ''){$sa_wh_s = get_option('bookmify_be_saturday_start', '08:00');}else{$saC1='data-checked="checked"';$saC2='checked';}
		if($sa_wh_e == ''){$sa_wh_e = get_option('bookmify_be_saturday_end', '18:00');}
		if($su_wh_s == ''){$su_wh_s = get_option('bookmify_be_sunday_start', '08:00');}else{$suC1='data-checked="checked"';$suC2='checked';}
		if($su_wh_e == ''){$su_wh_e = get_option('bookmify_be_sunday_end', '18:00');}
		...
		if($employee_id == ''){
			...
			foreach($categories as $category){
				...
				foreach($services as $service){
					...
					$employee_id 	= esc_sql($employee_id);
					$query 			= "SELECT capacity_min,capacity_max,price FROM {$wpdb->prefix}bmify_employee_services WHERE employee_id=".$employee_id." AND service_id=".$serviceID;
	...
	*/
	...
	public static function workingHoursListNano($day, $class = NULL, $start = NULL, $id = NULL){
		...
		$timeDiv		= '<div class="nano scrollbar-inner '.$day.' '.$class.'" data-id="'.$id.'" data-start="'.$start.'" data-class="'.$day.'"><div class="nano-content">';
		$startTime = strtotime('00:00');
		for($i = 0; $i < $countInterval; $i++){
			$timeDiv .= '<div>'.date("H:i", strtotime('+'.$i*$timeInterval.' minutes', $startTime)).'</div>';
		}
		$timeDiv .= '<div>24:00</div>';
		$timeDiv .= '</div></div>';
		return $timeDiv;
	}
