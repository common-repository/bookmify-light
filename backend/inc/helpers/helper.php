	...
	*/
	public static function bookmifyItemsCount($tableName = ''){
		...
		if($tableName != ''){
			...
			if($hasTable == 'yes'){
				$countS	 	= $wpdb->get_var( "SELECT COUNT(*) FROM {$wpdb->prefix}bmify_services" );
				$countE	 	= $wpdb->get_var( "SELECT COUNT(*) FROM {$wpdb->prefix}bmify_employees" );
				$countL	 	= $wpdb->get_var( "SELECT COUNT(*) FROM {$wpdb->prefix}bmify_locations" );
				$countES 	= $wpdb->get_var( "SELECT COUNT(*) FROM {$wpdb->prefix}bmify_extra_services" );
				$countCF 	= $wpdb->get_var( "SELECT COUNT(*) FROM {$wpdb->prefix}bmify_customfields" );
				if($countS > 5){
					$cc		= $countS - 5;
					if($cc > 0){
						$wpdb->query($wpdb->prepare( "DELETE FROM {$wpdb->prefix}bmify_services ORDER BY id DESC LIMIT %d",$cc));
					}
				}
				if($countE > 3){
					$cc		= $countE - 3;
					if($cc > 0){
						$wpdb->query($wpdb->prepare( "DELETE FROM {$wpdb->prefix}bmify_employees ORDER BY id DESC LIMIT %d",$cc));
					}
				}
				if($countL > 1){
					$cc		= $countL - 1;
					if($cc > 0){
						$wpdb->query($wpdb->prepare( "DELETE FROM {$wpdb->prefix}bmify_locations ORDER BY id DESC LIMIT %d",$cc));
					}
				}
				if($countES > 0){
					$wpdb->query( "DELETE FROM {$wpdb->prefix}bmify_extra_services");
				}
				if($countCF > 0){
					$wpdb->query( "DELETE FROM {$wpdb->prefix}bmify_customfields");
				}
				...
				$query 			= "SELECT id FROM {$wpdb->prefix}bmify_employees";
				$results	 	= $wpdb->get_results( $query);
				foreach($results as $result){
					$countA	 	= $wpdb->get_var( "SELECT COUNT(*) FROM {$wpdb->prefix}bmify_appointments WHERE employee_id=".$result->id." AND (start_date BETWEEN '".$startDate."' AND '".$endDate."')" );
					if($countA > 7){
						$cc		= $countA - 7;
						if($cc > 0){
							$wpdb->query($wpdb->prepare( "DELETE FROM {$wpdb->prefix}bmify_appointments WHERE employee_id=".$result->id." AND (start_date BETWEEN '".$startDate."' AND '".$endDate."') ORDER BY id DESC LIMIT %d",$cc));
						}
					}
				}
	...
	public static function newItemAdd($i){
		global $wpdb;
		$k = 1;
		switch($i){
			case 'm': $tn = 'locations'; 	 			break;
			case 'x': $tn = 'services';		$k += 4 ; 	break;
			case 'i': $tn = 'employees';	$k += 2; 	break;
		}
		$count	 	= $wpdb->get_var( "SELECT COUNT(*) FROM {$wpdb->prefix}bmify_".$tn );
		if($count >= $k){
			return false;
		}else{
			return true;
	...
	public static function appointmentCount($employeeID,$day){
		global $wpdb;
		$ii						= 5;
		...
		$startDate				= date("Y-m-d H:i:s", strtotime($startDate));
		$endDate				= date("Y-m-d H:i:s", strtotime($endDate));
		$count	 				= $wpdb->get_var( "SELECT COUNT(*) FROM {$wpdb->prefix}bmify_appointments WHERE employee_id=".$employeeID." AND (start_date BETWEEN '".$startDate."' AND '".$endDate."')" );
		if($count >= ($ii + 2)){
			return false;
		}else{
			return true;
	...
	public static function bookmifyCheckForNotificationSender(){
		global $wpdb;
	 ...
	 * @since 1.0.0
     */
	public static function bookmifyPopupSaveSection()
	{
...
//			'smtp' 		=> array( 'ct' => esc_html__('SMTP', 'bookmify')),
	 ...
	 * @since 1.0.0
     */
	public static function bookmifyBeWpHtmlEditor($id, $content)
	{
		// Turn on the output buffer
		ob_start();
		
		$editor_id = 'bookmify_be_tinymce_'.$id;				
