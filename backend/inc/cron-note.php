...
class CronNotification
{
	...
	public function bookmifySendReminder($receiver = '', $cron = ''){
		...
		$filterByService 	= esc_sql($notificationType);
		$query 		 		= "SELECT id,time_interval,check_time FROM {$wpdb->prefix}bmify_notifications WHERE type='".$notificationType."'";
			...
			if($receiver == 'customer'){
				...
				if(!empty($apps)){
					foreach($apps as $app){
						...
						$infoObject->customer_count	 	= count($customerIDs);
						$infoObject->appID	 			= $app->appID;
		...
		if($doCheck == 'do'){
			...
			$query 		 = "SELECT
								...
								FROM 	   	   {$wpdb->prefix}bmify_customer_appointments ca 
									...
									LEFT JOIN  {$wpdb->prefix}bmify_paymnets p					ON p.id = ca.payment_id
