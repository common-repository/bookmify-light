...
class Customers{
				...
				<div class="bookmify_be_customers">
					...
					<div class="bookmify_be_customers_list">
						<div class="bookmify_be_customer_list_content">
							<?php echo wp_kses_post($this->customersList()); ?>
						</div>
					</div>
					<!-- /Customer List -->
	...
	*/
	public function ajaxQueryInsertOrUpdateCustomer(){
		...
		if (!empty($_POST['bookmify_data'])) {
			...
			$c_fname 		= esc_sql($customer->first_name);
			$c_lname 		= esc_sql($customer->last_name);
			$c_email 		= esc_sql($customer->email);
			$c_phone 		= esc_sql($customer->phone);
			$c_birthday 	= esc_sql($customer->birthday);
			$c_country 		= esc_sql($customer->country);
			$c_state	 	= esc_sql($customer->state);
			$c_city	 		= esc_sql($customer->city);
			$c_address	 	= esc_sql($customer->address);
			$c_postcode 	= esc_sql($customer->postcode);
			$c_info 		= esc_sql($customer->info);
			
			$c_wp_user_id	= esc_sql($customer->wp_user_id);
			$demoWPID		= esc_sql($customer->wp_user_id);
	...
	*/
	public function ajaxQueryEditCustomer(){
		...
		if (!empty(esc_sql($_POST['bookmify_data']))) {
			$isAjaxCall = true;
			$id 		= esc_sql($_POST['bookmify_data']);
	...
	*/
	...
	public function ajaxFilterCustomerList(){
		...
		if (!empty(esc_sql($_POST['bookmify_page']))) {
			$isAjaxCall 	= true;
			$page 			= esc_sql($_POST['bookmify_page']);
			$search_text 	= esc_sql($_POST['bookmify_search']);
			$order 			= esc_sql($_POST['bookmify_order']);
	...
	*/
	public function ajaxQueryDeleteCustomer(){
		...
		if (!empty(esc_sql($_POST['bookmify_data']))) {
			$isAjaxCall		= true;
			$customerID 	= esc_sql($_POST['bookmify_data']);
			...
			if($count == 0){
				...
				$wpdb->query($wpdb->prepare( "DELETE FROM {$wpdb->prefix}bmify_appointments WHERE customer_id=%d", $customerID));
			}
