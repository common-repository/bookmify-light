	...
	*/
	public function ajaxFilterAppointmentList(){		
		...
		if ((!empty($_POST['bookmify_page']))) {
			$isAjaxCall 		= true;
			$page 				= esc_html($_POST['bookmify_page']);
	...
	public function ajaxQuerydayOffsEmployee(){
		...
		if (!empty(esc_html($_POST['bookmify_employee_id']))) {
			$isAjaxCall			= true;
			$employeeID 		= esc_html($_POST['bookmify_employee_id']);
	...
	public function ajaxQueryTimeSlotsAppointment(){
		...
		if (!empty(esc_html($_POST['serID']))) {
			$isAjaxCall			= true;
			$serviceID 			= esc_html($_POST['serID']);
			$appID 				= esc_html($_POST['appID']);
			$employeeID 		= esc_html($_POST['empID']);
			$dateValue 			= esc_html($_POST['dateVal']);
			$selDayBetween 		= esc_html($_POST['selDayBetween']);
			$newHours 			= esc_html($_POST['newHours']);
			$newMinutes 		= esc_html($_POST['newMinutes']);
			$selectedDayIndex 	= date('N', strtotime($dateValue));
			$extraDuration 		= esc_html($_POST['extraDuration']);
			if(!$extraDuration){
				$extraDuration 	= 0;
			}
...
//			$summaryDuration 		= ($serviceDuration+$serviceBuffAfter+$extraDuration) / 60; // в минутах
			...
			if(!$appCount){
				$html .= '<div class="nodata">'.esc_html__('You reached the limit', 'bookmify').'</div>';
			}else{
	...
	public function ajaxQueryGetExtrasOfServiceForAppointment(){
		...
		if (!empty(esc_html($_POST['serviceID']))) {
			$isAjaxCall			= true;
			$customerIDs 		= esc_html($_POST['customerIDs']);
			$serviceID 			= esc_html($_POST['serviceID']);
			$employeeID 		= esc_html($_POST['employeeID']);
			if(!empty($_POST['customerData'])){
				$customerData 	= esc_sql($_POST['customerData']);
			}
	...
	public function ajaxQueryGetPaymentsCustomersForAppointment(){
		...
		if (!empty(esc_html($_POST['serviceID']))) {
			...
			if(!empty(esc_sql($_POST['customerData']))){
				$customerData 	= esc_sql($_POST['customerData']);
			}
			
			$html				= HelperAppointments::getPaymentAgain($customerIDs,$serviceID,$employeeID,$customerData);
	...
	public function ajaxQueryEditAppointment(){
		...
		$serviceID 		= '';
		$employeeID 	= '';
		if (!empty(esc_html($_POST['bookmify_data']))) {
			$isAjaxCall = true;
			$id 		= esc_html($_POST['bookmify_data']);
			...
			$today					= HelperTime::getCurrentDateTime();
			if($appointmentStartDate <= $today){ // if closed item do nothing
				$html				= 'warning';
			}else{
	...
	public function ajaxQueryInsertOrUpdateAppointment(){
		...
		// **************************************************************************************************************************
		if (esc_html($_POST['insertOrUpdate']) 	== 'update') {
			...
			if (!empty($_POST['details'])) {
				...
				$timeAppEnd				= date("H:i:s",strtotime($timeApp) + esc_html($_POST['duration']));
				...
				if($checked == 'public'){
					...
					if($oldEmployeeID == $empID){
						...
						if($oldServiceID != $serID){
							...
							$infoObject->userID				= $oldEmployeeID;
			...
			if (!empty($_POST['allCustomers'])) {
				...
				foreach($allCustomers as $customer){
					...
					if(esc_html($_POST['haveExtras']) == "yes"){
		...
		if(!empty(esc_html($_POST['do']))){
			...
			if (!empty($_POST['bookmify_daterange'])) {
				$filterDateRange 	= $_POST['bookmify_daterange'];
			}
	...
	*/
	public function ajaxQueryDeleteAppointment(){
		...
		if (!empty(esc_html($_POST['ID']))){
			$isAjaxCall = true;
			$appID 		= esc_html($_POST['ID']);
